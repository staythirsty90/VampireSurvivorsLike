...
{
    public readonly struct TypeHandleFieldDescription : IEquatable<TypeHandleFieldDescription>,
        INonQueryFieldDescription
    {
        ...
        {
            TypeHandleSource.Aspect =>
                $"{"[global::Unity.Collections.ReadOnly] ".EmitIfTrue(IsReadOnly)} {(forcePublic ? "public" : "")} {TypeSymbol.ToFullName()}.TypeHandle {GeneratedFieldName};",
            TypeHandleSource.BufferElement =>
                $"{"[global::Unity.Collections.ReadOnly] ".EmitIfTrue(IsReadOnly)} {(forcePublic ? "public" : "")} Unity.Entities.BufferTypeHandle<{TypeSymbol.ToFullName()}> {GeneratedFieldName};",
            TypeHandleSource.Component =>
                $"{"[global::Unity.Collections.ReadOnly] ".EmitIfTrue(IsReadOnly)} {(forcePublic ? "public" : "")} Unity.Entities.ComponentTypeHandle<{TypeSymbol.ToFullName()}> {GeneratedFieldName};",
            _ =>
                $"{"[global::Unity.Collections.ReadOnly] ".EmitIfTrue(IsReadOnly)} {(forcePublic ? "public" : "")} Unity.Entities.SharedComponentTypeHandle<{TypeSymbol.ToFullName()}> {GeneratedFieldName};",
        };
            ...
            else
            {
                ...
                switch (forcedTypeHandleSource)
                {
                    case TypeHandleSource.Aspect:
                        Debug.Assert(
                            constraintTypes.Any(t =>
                                t.ToFullName() == "global::Unity.Entities.IAspect" || t.IsAspect()),
                            "SG-DBG: Specified aspect types must be aspects");
                        GeneratedFieldName =
                            $"__{typeSymbolValidIdentifier}_{(IsReadOnly ? "RO" : "RW")}_AspectTypeHandle";
                        Source = TypeHandleSource.Aspect;
                        break;
                    case TypeHandleSource.BufferElement:
                        ...
                        GeneratedFieldName =
                            $"__{typeSymbolValidIdentifier}_{(IsReadOnly ? "RO" : "RW")}_BufferTypeHandle";
                        Source = TypeHandleSource.BufferElement;
                        break;
                    case TypeHandleSource.SharedComponent:
                        Debug.Assert(
                            constraintTypes.Any(t =>
                                t.ToFullName() == "global::Unity.Entities.ISharedComponentData" ||
                                t.IsSharedComponent()),
